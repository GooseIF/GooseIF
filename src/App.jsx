...
function App() {
  ...
  async function handleClick() {
    const coords = await fetchLocation(baseRef.current.value).then(data => data.json()).then((data) => {
      const result = [data[0].lat, data[0].lon]
      return result
    })
   const weatherInfo = await fetchWeather(coords[0], coords[1]).then(data => data.json())
